import { P4Interface } from "./class/P4Interface.js";
import { Player, GameRoomList } from "./class/gameRoom.js";

export const rooms = new GameRoomList(2, P4Interface);
const regType = /^[a-z]+$/i;

/**
 * @param {Player} player
 * @returns {{playerId:number, cPlayer:number, board:number[]?}}
 */
...
export const websocketConnection = async (socket, req) => {
	...
	socket.on("game-join", async (data) => {
		...
		try {
			...
			player.send("info", `Impossible de rejoindre la Salle #${data}, ${error.message}`);
